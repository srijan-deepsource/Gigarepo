 ...
 */
function statusEnd(code = 400, response) {
  // <- default params should come last
...
function capitalOfCountry(country) {
  ...
  switch (country) {
    case "India":
      capital = "New Delhi"; // <- no break or fallthrough comment
    case "USA":
...
app.get("/exec", async (req, res) => {
  try {
    ...
    if (typeof code !== "strign") {
      // <- bad comparison with `typeof`
      statusEnd(400, res);
